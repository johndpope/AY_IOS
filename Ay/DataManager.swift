                    ...
                    if event.recur_end == nil {
                        ...
                        while (event.recur_occur > 0 && event.recur_occur > index) || (event.recur_occur == 0 && index == 1) {
                            ...
                            if isInThisMonth(next_start_components, end_components: next_end_components, month: month, year: year) {
                                var schedule = AyEvent(id: event.id, target_name: event.target_name, start: next_start_date, end: next_end_date, title: event.title, alarm: event.alarm_time, recur_end: event.recur_end, recur_freq: event.recur_freq, recur_occur: event.recur_occur, participants : nil, type : "", loc: "")
